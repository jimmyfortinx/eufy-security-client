...
export class P2PClientProtocol extends TypedEmitter<P2PClientProtocolEvents> {
  ...
  private readonly MAX_EXPECTED_SEQNO_WAIT = process.env.MAX_EXPECTED_SEQNO_WAIT
    ? parseInt(process.env.MAX_EXPECTED_SEQNO_WAIT)
    : 20 * 1000;
  ...
  private readonly MAX_STREAM_DATA_WAIT = process.env.MAX_STREAM_DATA_WAIT
    ? parseInt(process.env.MAX_STREAM_DATA_WAIT)
    : 5 * 1000;
  ...
  private energySavingDeviceP2PSeqMapping: Map<number, number> = new Map<
    number,
    number
  >();
  ...
  private _initialize(): void {
    ...
    for (let datatype = 0; datatype < 4; datatype++) {
      ...
      if (datatype === P2PDataType.VIDEO) rsaKey = getNewRSAPrivateKey();
      else rsaKey = null;
  ...
  private async sendMessage(
    ...
    await sendMessage(this.socket, address, msgID, payload).catch((error) => {
      this.log.error(
        `${errorSubject} - msgID: ${msgID.toString(
          "hex"
        )} payload: ${payload?.toString("hex")} - Error:`,
        error
      );
    });
  }
  ...
  private _disconnected(): void {
    ...
    this.sendQueue = this.sendQueue.filter(
      (queue) =>
        queue.commandType !== CommandType.CMD_PING &&
        queue.commandType !== CommandType.CMD_GET_DEVICE_PING
    );
    if (this.connected) {
      this.emit("close");
  ...
  private closeEnergySavingDevice(): void {
    if (
      ...
      if (this.esdDisconnectTimeout === undefined) {
        this.log.debug(
          `Station ${this.rawStation.station_sn} - Energy saving device - No more p2p commands to execute or running streams, initiate disconnect timeout in ${this.ESD_DISCONNECT_TIMEOUT} milliseconds...`
        );
  ...
  private async renewDSKKey(): Promise<void> {
    if (
      ...
      this.log.debug(
        `Station ${this.rawStation.station_sn} DSK keys not present or expired, get/renew it`,
        { dskKey: this.dskKey, dskExpiration: this.dskExpiration }
      );
  ...
  private cloudLookup2(origAddress: Address, data: Buffer): void {
    this.cloudAddresses.map((address) =>
      this.cloudLookupByAddress3(address, origAddress, data)
    );
  }
  ...
  private async localLookupByAddress(address: Address): Promise<void> {
    ...
    await this.sendMessage(
      `Local lookup address for station ${this.rawStation.station_sn}`,
      address,
      msgId,
      payload
    );
  ...
  private async cloudLookupByAddress(address: Address): Promise<void> {
    ...
    await this.sendMessage(
      `Cloud lookup addresses for station ${this.rawStation.station_sn}`,
      address,
      msgId,
      payload
    );
  ...
  private async cloudLookupByAddress2(address: Address): Promise<void> {
    ...
    await this.sendMessage(
      `Cloud lookup addresses (2) for station ${this.rawStation.station_sn}`,
      address,
      msgId,
      payload
    );
  ...
  private async cloudLookupByAddress3(
    address: Address,
    origAddress: Address,
    data: Buffer
  ): Promise<void> {
    ...
    await this.sendMessage(
      `Cloud lookup addresses (3) for station ${this.rawStation.station_sn}`,
      address,
      msgId,
      payload
    );
  ...
  private _startConnectTimeout(): void {
    if (this.connectTimeout === undefined)
      this.connectTimeout = setTimeout(() => {
        this.log.warn(
          `Station ${this.rawStation.station_sn} - Tried all hosts, no connection could be established`
        );
        this._disconnected();
  ...
  private _connect(address: Address, p2p_did: string): void {
    this.log.debug(
      `Station ${this.rawStation.station_sn} - CHECK_CAM - Connecting to host ${address.host} on port ${address.port}...`
    );
    for (let i = 0; i < 4; i++) this.sendCamCheck(address, p2p_did);
  ...
  private lookup(host?: string): void {
    ...
    this.lookupTimeout = setTimeout(() => {
      this.lookupTimeout = undefined;
      this.log.error(
        `Station ${this.rawStation.station_sn} - All address lookup tentatives failed.`
      );
      ...
      if (!this.binded)
        this.socket.bind(0, () => {
          ...
          try {
            ...
            this.log.error(`Station ${this.rawStation.station_sn} - Error:`, {
              error: error,
  ...
  private async sendCamCheck(address: Address, p2p_did: string): Promise<void> {
    ...
    await this.sendMessage(
      `Send cam check to station ${this.rawStation.station_sn}`,
      address,
  ...
  private async sendCamCheck2(address: Address, data: Buffer): Promise<void> {
    ...
    await this.sendMessage(
      `Send cam check (2) to station ${this.rawStation.station_sn}`,
      address,
  ...
  public async sendPing(address: Address): Promise<void> {
    if (
      ...
      if (!this.energySavingDevice)
        this.log.warn(
          `Station ${this.rawStation.station_sn} - Heartbeat check failed. Connection seems lost. Try to reconnect...`
        );
      this._disconnected();
    }
    await this.sendMessage(
      `Send ping to station ${this.rawStation.station_sn}`,
      address,
  ...
  public sendCommandWithIntString(
    ...
    const payload = buildIntStringCommandPayload(
      p2pcommand.value,
      p2pcommand.valueSub === undefined ? 0 : p2pcommand.valueSub,
      p2pcommand.strValue === undefined ? "" : p2pcommand.strValue,
      p2pcommand.strValueSub === undefined ? "" : p2pcommand.strValueSub,
      p2pcommand.channel
    );
    if (p2pcommand.commandType === CommandType.CMD_NAS_TEST) {
      this.currentMessageState[P2PDataType.DATA].rtspStream[
        p2pcommand.channel
      ] = p2pcommand.value === 1 ? true : false;
    }
    this.sendCommand(
  ...
  public sendCommandWithInt(
    ...
    const payload = buildIntCommandPayload(
      p2pcommand.value,
      p2pcommand.strValue === undefined ? "" : p2pcommand.strValue,
      p2pcommand.channel
    );
  ...
  public sendCommandWithStringPayload(
    ...
    const payload = buildCommandWithStringTypePayload(
      p2pcommand.value,
      p2pcommand.channel
    );
    ...
    if (p2pcommand.commandType == CommandType.CMD_SET_PAYLOAD) {
      try {
        ...
        this.log.error(
          `CMD_SET_PAYLOAD - Station ${this.rawStation.station_sn} - Error:`,
          error
        );
      }
      ...
      try {
        ...
        this.log.error(
          `CMD_DOORBELL_SET_PAYLOAD - Station ${this.rawStation.station_sn} - Error:`,
          error
        );
      }
    }
  ...
  public sendCommandWithString(
    ...
    const payload = buildStringTypeCommandPayload(
      p2pcommand.strValue,
      p2pcommand.strValueSub,
      p2pcommand.channel
    );
    this.sendCommand(
      p2pcommand.commandType,
      ...
      if (this.connected) {
        ...
        while ((queuedMessage = this.sendQueue.shift()!) !== undefined) {
          ...
          this.messageStates.forEach((stateMessage) => {
            if (
              stateMessage.commandType === queuedMessage.commandType &&
  ...
  private sendCommand(
    ...
    if (
      message.commandType !== CommandType.CMD_PING &&
      message.commandType !== CommandType.CMD_GET_DEVICE_PING
    )
      this._clearESDDisconnectTimeout();
      ...
      messageState.retryTimeout = setTimeout(() => {
        if (this.connectAddress) {
          sendMessage(
            ...
            this.log.error(
              `Station ${this.rawStation.station_sn} - Error:`,
              error
            );
          });
  ...
  private async _sendCommand(
    ...
    if (isP2PQueueMessage(message)) {
      ...
      if (ageing <= this.MAX_COMMAND_QUEUE_TIMEOUT) {
        ...
        this.log.warn(
          `Station ${this.rawStation.station_sn} - Command aged out from queue`,
          {
            commandType: message.commandType,
            nestedCommandType: message.nestedCommandType,
            channel: message.channel,
      ...
      if (
        ...
        this.log.error(
          `Station ${this.rawStation.station_sn} - Max retries ${
            this.messageStates.get(message.sequence)?.retries
          } - stop with error ${ErrorCode[message.returnCode]}`,
          {
    ...
    if (
      messageState.commandType !== CommandType.CMD_PING &&
      this.energySavingDevice
    ) {
      this.energySavingDeviceP2PSeqMapping.set(
        this.energySavingDeviceP2PDataSeqNumber,
        message.sequence
      );
      this.log.debug(
        `Station ${this.rawStation.station_sn} - Energy saving Device - Added sequence number mapping`,
        {
          commandType: message.commandType,
          seqNumber: message.sequence,
          energySavingDeviceP2PDataSeqNumber:
            this.energySavingDeviceP2PDataSeqNumber,
          energySavingDeviceP2PSeqMappingCount:
            this.energySavingDeviceP2PSeqMapping.size,
        }
      );
      this.energySavingDeviceP2PDataSeqNumber = this._incrementSequence(
        this.energySavingDeviceP2PDataSeqNumber
      );
    }

    this.log.debug("Sending p2p command...", {
      station: this.rawStation.station_sn,
    ...
    await this.sendMessage(
      `Send command to station ${this.rawStation.station_sn}`,
      this.connectAddress!,
      RequestMessageType.DATA,
      messageState.data
    );
    ...
    if (messageState.retries === 0) {
      if (
        ...
        (messageState.nestedCommandType !== undefined &&
        ...
        (messageState.nestedCommandType !== undefined &&
          messageState.nestedCommandType === 1000 &&
    ...
    if (hasHeader(msg, ResponseMessageType.LOCAL_LOOKUP_RESP)) {
      if (!this.connected) {
        ...
        const p2pDid = `${msg
          ...
          .replace(/[\0]+$/g, "")}`;
        this.log.debug(
          `Station ${this.rawStation.station_sn} - LOCAL_LOOKUP_RESP - Got response`,
          { ip: rinfo.address, port: rinfo.port, p2pDid: p2pDid }
        );

        if (p2pDid === this.rawStation.p2p_did) {
          this.log.debug(
            `Station ${this.rawStation.station_sn} - LOCAL_LOOKUP_RESP - Wanted device was found, connect to it`,
            { ip: rinfo.address, port: rinfo.port, p2pDid: p2pDid }
          );
          this._connect({ host: rinfo.address, port: rinfo.port }, p2pDid);
        } else {
          this.log.debug(
            `Station ${this.rawStation.station_sn} - LOCAL_LOOKUP_RESP - Unwanted device was found, don't connect to it`,
            { ip: rinfo.address, port: rinfo.port, p2pDid: p2pDid }
          );
        }
      }
      ...
      if (!this.connected) {
        ...
        this.log.debug(
          `Station ${this.rawStation.station_sn} - LOOKUP_ADDR - Got response`,
          {
        ...
        if (ip === "0.0.0.0") {
          this.log.debug(
            `Station ${this.rawStation.station_sn} - LOOKUP_ADDR - Got invalid ip address 0.0.0.0, ignoring response...`
          );
          return;
        }
        ...
        if (this.connectionType === P2PConnectionType.ONLY_LOCAL) {
          if (isPrivateIp(ip)) {
            ...
            this._clearLookupRetryTimeout();
            this.log.debug(
              `Station ${this.rawStation.station_sn} - ONLY_LOCAL - Try to connect to ${ip}:${port}...`
            );
          ...
          this._clearLookupRetryTimeout();
          this.log.debug(
            `Station ${this.rawStation.station_sn} - QUICKEST - Try to connect to ${ip}:${port}...`
          );
          this._connect({ host: ip, port: port }, this.rawStation.p2p_did);
        }
      }
      ...
      hasHeader(msg, ResponseMessageType.CAM_ID2)
    ) {
      // Answer from the device to a CAM_CHECK message
      if (!this.connected) {
        this.log.debug(
          `Station ${this.rawStation.station_sn} - CAM_ID - Connected to station ${this.rawStation.station_sn} on host ${rinfo.address} port ${rinfo.port}`
        );
        this._clearLookupRetryTimeout();
        this._clearLookupTimeout();
        this._clearConnectTimeout();
        ...
        if (
          Device.isLockWifi(this.rawStation.device_type) ||
          Device.isLockWifiNoFinger(this.rawStation.device_type)
        ) {
          ...
          this.sendMessage(
            `Send smartsafe gateway command to station ${this.rawStation.station_sn}`,
            this.connectAddress!,
            RequestMessageType.DATA,
            data
          );
          ...
          const data = Buffer.concat([
            ...
            buildCommandHeader(0, CommandType.CMD_PING).slice(2),
            payload,
          ]);
          const message: P2PMessageState = {
            sequence: this.seqNumber,
          ...
          this.seqNumber = this._incrementSequence(this.seqNumber);
          this.sendMessage(
            `Send lock wifi gateway command to station ${this.rawStation.station_sn}`,
            this.connectAddress!,
            RequestMessageType.DATA,
            data
          );
          try {
            ...
            this.log.error(
              `Send query status lock command to station ${this.rawStation.station_sn} - Error`,
              error
            );
          }
          tmpSendQueue.forEach((element) => {
      ...
      if (msg.length > 4) this.lastPongData = msg.slice(4);
      ...
      this.sendMessage(
        `Send pong to station ${this.rawStation.station_sn}`,
      ...
      this.log.debug(
        `Station ${this.rawStation.station_sn} - END - received from host ${rinfo.address}:${rinfo.port}`
      );
      this.onClose();
      return;
      ...
      for (let i = 1; i <= numAcks; i++) {
        ...
        this.log.debug(
          `Station ${this.rawStation.station_sn} - ACK ${P2PDataType[dataType]} - received from host ${rinfo.address}:${rinfo.port} for sequence ${ackedSeqNo}`
        );
            ...
            if (
              ...
              if (
                ...
                msg_state.timeout = setTimeout(() => {
                  ...
                  this.log.warn(
                    `Station ${this.rawStation.station_sn} - Result data for command not received`,
                    {
                      message: {
      ...
      if (this.connected) {
        ...
        this.log.debug(
          `Station ${this.rawStation.station_sn} - DATA ${
            P2PDataType[message.type]
          } - received from host ${rinfo.address}:${
            rinfo.port
          } - Processing sequence ${message.seqNo}...`
        );
        ...
        if (message.seqNo === this.expectedSeqNo[dataType]) {
          ...
          this.log.debug(
            `Station ${this.rawStation.station_sn} - DATA ${
              P2PDataType[message.type]
            } - Received expected sequence (expectedSeqNo: ${
              this.expectedSeqNo[dataType]
            } seqNo: ${message.seqNo} queuedData.size: ${
          ...
          while (queuedMessage) {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - DATA ${
                P2PDataType[queuedMessage.type]
              } - Work off queued data (expectedSeqNo: ${
                this.expectedSeqNo[dataType]
              } seqNo: ${queuedMessage.seqNo} queuedData.size: ${
                this.currentMessageState[dataType].queuedData.size
              })`
            );
            this.expectedSeqNo[dataType] = this._incrementSequence(
          ...
          this.log.debug(
            `Station ${this.rawStation.station_sn} - DATA ${
              P2PDataType[message.type]
            } - Received already processed sequence (expectedSeqNo: ${
              this.expectedSeqNo[dataType]
            } seqNo: ${message.seqNo} queuedData.size: ${
              this.currentMessageState[dataType].queuedData.size
            })`
          ...
          if (
            ...
            this.log.debug(
              `Station ${this.rawStation.station_sn} - DATA ${
                P2PDataType[message.type]
              } - Received not expected sequence, added to the queue for future processing (expectedSeqNo: ${
                this.expectedSeqNo[dataType]
              } seqNo: ${message.seqNo} queuedData.size: ${
                this.currentMessageState[dataType].queuedData.size
              })`
            ...
            this.log.debug(
              `Station ${this.rawStation.station_sn} - DATA ${
                P2PDataType[message.type]
              } - Received not expected sequence, discarded since already present in queue for future processing (expectedSeqNo: ${
                this.expectedSeqNo[dataType]
              } seqNo: ${message.seqNo} queuedData.size: ${
                this.currentMessageState[dataType].queuedData.size
              })`
      ...
      if (!this.connected) {
        ...
        this.log.debug(
          `Station ${this.rawStation.station_sn} - LOOKUP_ADDR2 - Got response`,
          {
            remoteAddress: rinfo.address,
            remotePort: rinfo.port,
        ...
        this.log.debug(
          `Station ${this.rawStation.station_sn} - CHECK_CAM2 - Connecting to host ${ip} on port ${port}...`
        );
        ...
        this._startConnectTimeout();
        this.sendMessage(
          `Send UNKNOWN_70 to station ${this.rawStation.station_sn}`,
          { host: ip, port: port },
          RequestMessageType.UNKNOWN_70
        );
      }
    } else if (hasHeader(msg, ResponseMessageType.UNKNOWN_71)) {
      if (!this.connected) {
        this.log.debug(
          `Station ${this.rawStation.station_sn} - UNKNOWN_71 - Got response`,
          {
            remoteAddress: rinfo.address,
            remotePort: rinfo.port,
        ...
        this.sendMessage(
          `Send UNKNOWN_71 to station ${this.rawStation.station_sn}`,
          { host: rinfo.address, port: rinfo.port },
          RequestMessageType.UNKNOWN_71
        );
      }
      ...
      if (!this.connected) {
        ...
        this.log.debug(
          `Station ${this.rawStation.station_sn} - UNKNOWN_73 - Got response`,
          {
            remoteAddress: rinfo.address,
            remotePort: rinfo.port,
            response: { port: port, data: data.toString("hex") },
          }
        );
        this.cloudLookup2({ host: rinfo.address, port: port }, data);
      }
    } else if (
      hasHeader(msg, ResponseMessageType.UNKNOWN_81) ||
      ...
      if (!this.connected) {
        ...
        this.log.debug(
          `Station ${this.rawStation.station_sn} - LOOKUP_RESP - Got response`,
          {
            remoteAddress: rinfo.address,
            remotePort: rinfo.port,
      ...
      this.log.debug(
        `Station ${this.rawStation.station_sn} - received unknown message`,
        {
          remoteAddress: rinfo.address,
          remotePort: rinfo.port,
          response: { message: msg.toString("hex"), length: msg.length },
  ...
  private parseDataMessage(message: P2PMessage): void {
    if (
      ...
      this.log.debug(
        `Station ${this.rawStation.station_sn} - DATA ${
          P2PDataType[message.type]
        } - Stream not started ignore this data`,
        {
          seqNo: message.seqNo,
      ...
      do {
        ...
        this.log.debug(
          `Station ${this.rawStation.station_sn} - Received data`,
          {
            seqNo: message.seqNo,
            header: this.currentMessageBuilder[message.type].header,
            bytesRead: this.currentMessageBuilder[message.type].bytesRead,
        ...
        if (
          ...
          if (data.length > 0 && message.type === P2PDataType.DATA) {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - Parsed data`,
              {
                seqNo: message.seqNo,
  ...
  private handleData(message: P2PDataMessage): void {
    if (message.dataType === P2PDataType.CONTROL) {
      ...
      if (result_msg) {
        ...
        if (this.energySavingDevice) {
          const goodSeqNumber = this.energySavingDeviceP2PSeqMapping.get(
            message.seqNo
          );
          if (goodSeqNumber) {
            ...
            this.log.debug(
              `Station ${this.rawStation.station_sn} - Energy saving Device - Result data received - Detecting correct sequence number`,
              {
                commandIdName: commandStr,
                commandId: message.commandId,
        ...
        if (msg_state) {
          if (msg_state.commandType === message.commandId) {
            ...
            const command_type =
              msg_state.nestedCommandType !== undefined
                ...
                : msg_state.commandType;
            this.log.debug(
              `Station ${this.rawStation.station_sn} - Result data for command received`,
              {
                message: {
                  sequence: msg_state.sequence,
                  commandType: msg_state.commandType,
            ...
            if (return_code === ErrorCode.ERROR_FAILED_TO_REQUEST) {
              ...
              if (
                ...
                this.secondaryCommandTimeout = setTimeout(() => {
                  this.log.warn(
                    `Station ${this.rawStation.station_sn} - Result data for secondary command not received`,
                    {
                      message: {
                        sequence: msg_state!.sequence,
                        commandType: msg_state!.commandType,
              ...
              if (
                ...
                (msg_state.nestedCommandType !== undefined &&
                  msg_state.nestedCommandType === 1000 &&
                  msg_state.commandType ===
                ...
                if (return_code === ErrorCode.ERROR_PPCS_SUCCESSFUL) {
                  ...
                  this.emit(
                    ...
                    msg_state.channel,
                    new TalkbackError(
                      `Station ${this.rawStation.station_sn} channel ${msg_state.channel} someone is responding now.`
                    )
                  );
                  ...
                  this.emit(
                    ...
                    msg_state.channel,
                    new TalkbackError(
                      `Station ${this.rawStation.station_sn} channel ${msg_state.channel} wait a second, device is busy.`
                    )
                  );
                } else {
                  this.emit(
                    ...
                    msg_state.channel,
                    new TalkbackError(
                      `Station ${this.rawStation.station_sn} channel ${msg_state.channel} connect failed please try again later.`
                    )
                  );
                }
              } else if (
                ...
                this.emit(
                  "sd info ex",
                  message.data.slice(0, 4).readUInt32LE(),
                  message.data.slice(4, 8).readUInt32LE(),
                  message.data.slice(8, 12).readUInt32LE()
                );
              }
            }
          } else {
            this.messageStates.delete(message.seqNo);
            this.log.debug(
              `Station ${this.rawStation.station_sn} - dataType: ${
                P2PDataType[message.dataType]
              } commandtype and sequencenumber different!`,
              {
          ...
          this.log.debug(
            `Station ${this.rawStation.station_sn} - dataType: ${
              P2PDataType[message.dataType]
            } commandId: ${message.commandId} sequence: ${
              message.seqNo
            } not present!`,
            {
              seqNumber: this.seqNumber,
        ...
        this.log.debug(
          `Station ${this.rawStation.station_sn} - Unsupported response`,
          {
      ...
      this.log.debug(
        `Station ${this.rawStation.station_sn} - Not implemented data type`,
        {
          seqNo: message.seqNo,
  ...
  private waitForStreamData(dataType: P2PDataType): void {
    ...
    this.currentMessageState[dataType].p2pStreamingTimeout = setTimeout(() => {
      this.log.info(
        `Stopping the station stream for the device ${
          this.deviceSNs[this.currentMessageState[dataType].p2pStreamChannel]
            ?.sn
        }, because we haven't received any data for ${
    ...
    if (!this.currentMessageState[message.dataType].invalidStream) {
      switch (message.commandId) {
        case CommandType.CMD_VIDEO_FRAME:
          ...
          if (message.signCode > 0 && data_length >= 128) {
            ...
            if (rsaKey) {
              try {
                ...
                this.log.debug(
                  `Station ${this.rawStation.station_sn} - Decrypted AES key: ${videoMetaData.aesKey}`
                );
              } catch (error) {
                if (this.previousAesKey) {
                  this.log.debug(
                    `Station ${this.rawStation.station_sn} - Message bigger than expected ${data_length}. Reusing the previous key to find back the message.`
                  );
                  videoMetaData.aesKey = rsaKey
                    .decrypt(this.previousAesKey)
                    .toString("hex");
                  payloadStart = message.data.indexOf(this.previousAesKey) - 22;
                } else {
                  this.log.warn(
                    `Station ${this.rawStation.station_sn} - AES key could not be decrypted! The entire stream is discarded. - Error:`,
                    error
                  );
                  this.currentMessageState[message.dataType].invalidStream =
                    true;
                  this.emit(
                    "livestream error",
                    message.channel,
                    new LivestreamError(
                      `Station ${this.rawStation.station_sn} AES key could not be decrypted! The entire stream is discarded.`
                    )
                  );
                  return;
                }
              ...
              this.log.warn(
                `Station ${this.rawStation.station_sn} - Private RSA key is missing! Stream could not be decrypted. The entire stream is discarded.`
              );
          ...
          const isKeyFrame =
            message.data
              .slice(payloadStart + 4, payloadStart + 5)
              .readUInt8() === 1
              ? true
              : false;

          videoMetaData.videoDataLength = message.data
            .slice(payloadStart + 0, payloadStart + 4)
            .readUInt32LE();
          videoMetaData.streamType = message.data
            .slice(payloadStart + 5, payloadStart + 6)
            .readUInt8();
          videoMetaData.videoSeqNo = message.data
            .slice(payloadStart + 6, payloadStart + 8)
            .readUInt16LE();
          videoMetaData.videoFPS = message.data
            .slice(payloadStart + 8, payloadStart + 10)
            .readUInt16LE();
          videoMetaData.videoWidth = message.data
            .slice(payloadStart + 10, payloadStart + 12)
            .readUInt16LE();
          videoMetaData.videoHeight = message.data
            .slice(payloadStart + 12, payloadStart + 14)
            .readUInt16LE();
          videoMetaData.videoTimestamp = message.data
            .slice(payloadStart + 14, payloadStart + 20)
            .readUIntLE(0, 6);
          ...
          if (videoMetaData.aesKey !== "") {
            const encrypted_data = message.data.slice(
              payloadStart,
              payloadStart + 128
            );
            const unencrypted_data = message.data.slice(
              payloadStart + 128,
          ...
          this.log.debug(
            `Station ${this.rawStation.station_sn} - CMD_VIDEO_FRAME`,
            {
          ...
          if (
            ...
            if (
              ...
              this.log.debug(
                `Station ${this.rawStation.station_sn} - CMD_VIDEO_FRAME - Video codec information received from packet`,
                {
              ...
              this.log.debug(
                `Station ${this.rawStation.station_sn} - CMD_VIDEO_FRAME - Video codec extracted from video data`,
                {
                  commandIdName: CommandType[message.commandId],
                  commandId: message.commandId,
                  channel: message.channel,
              ...
              if (
                ...
                if (
                  ...
                  this.log.debug(
                    `Station ${this.rawStation.station_sn} - CMD_VIDEO_FRAME - Unknown video codec`,
                    {
                      commandIdName: CommandType[message.commandId],
                      commandId: message.commandId,
                      channel: message.channel,
                  ...
                  this.log.debug(
                    `Station ${this.rawStation.station_sn} - CMD_VIDEO_FRAME - Fallback, using video codec information received from packet`,
                    {
                      commandIdName: CommandType[message.commandId],
                      commandId: message.commandId,
                      channel: message.channel,
                ...
                this.log.debug(
                  `Station ${this.rawStation.station_sn} - CMD_VIDEO_FRAME - Fallback, video codec extracted from video data`,
                  {
                    commandIdName: CommandType[message.commandId],
                    commandId: message.commandId,
                    channel: message.channel,
          ...
          if (message.dataType === P2PDataType.VIDEO) {
            if (findStartCode(video_data)) {
              this.log.debug(
                `Station ${this.rawStation.station_sn} - CMD_VIDEO_FRAME: startcode found`,
                {
              ...
              if (
                ...
                this.log.debug(
                  `Station ${this.rawStation.station_sn} - CMD_VIDEO_FRAME: Skipping because first frame is not an I frame.`
                );
              }
            } else {
              this.log.debug(
                `Station ${this.rawStation.station_sn} - CMD_VIDEO_FRAME: No startcode found`,
                {
                  isKeyFrame: isKeyFrame,
                  preFrameVideoDataLength:
                    this.currentMessageState[message.dataType].preFrameVideoData
        ...
        case CommandType.CMD_AUDIO_FRAME:
          ...
          this.log.debug(
            `Station ${this.rawStation.station_sn} - CMD_AUDIO_FRAME`,
            {
              dataSize: message.data.length,
        ...
        default:
          this.log.debug(
            `Station ${this.rawStation.station_sn} - Not implemented message`,
            {
              commandIdName: CommandType[message.commandId],
              commandId: message.commandId,
              channel: message.channel,
      ...
      this.log.debug(
        `Station ${this.rawStation.station_sn} - Invalid stream data, dropping complete stream`,
        {
          commandIdName: CommandType[message.commandId],
          commandId: message.commandId,
          channel: message.channel,
    ...
    try {
      switch (message.commandId) {
        case CommandType.CMD_GET_ALARM_MODE:
          this.log.debug(
            `Station ${this.rawStation.station_sn} - Alarm mode changed to: ${
              AlarmMode[message.data.readUIntBE(0, 1)]
            }`
          );
        ...
        case CommandType.CMD_CAMERA_INFO:
          try {
            ...
            this.log.debug(
              `Station ${this.rawStation.station_sn} - Camera info`,
              { cameraInfo: data }
            );
            this.emit(
              "camera info",
              parseJSON(data, this.log) as CmdCameraInfoResponse
            );
          } catch (error) {
            this.log.error(
              `Station ${this.rawStation.station_sn} - Camera info - Error:`,
              error
            );
          }
          break;
        case CommandType.CMD_CONVERT_MP4_OK:
          ...
          this.log.debug(
            `Station ${this.rawStation.station_sn} - CMD_CONVERT_MP4_OK`,
            { channel: message.channel, totalBytes: totalBytes }
          );
        ...
        case CommandType.CMD_WIFI_CONFIG:
          ...
          this.log.debug(
            `Station ${this.rawStation.station_sn} - CMD_WIFI_CONFIG`,
            { channel: message.channel, rssi: rssi }
          );
          this.emit("wifi rssi", message.channel, rssi);
          break;
        case CommandType.CMD_DOWNLOAD_FINISH:
          this.log.debug(
            `Station ${this.rawStation.station_sn} - CMD_DOWNLOAD_FINISH`,
            { channel: message.channel }
          );
          this.endStream(P2PDataType.BINARY);
          break;
        case CommandType.CMD_DOORBELL_NOTIFY_PAYLOAD:
          try {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - CMD_DOORBELL_NOTIFY_PAYLOAD`,
            ...
            this.log.error(
              `Station ${this.rawStation.station_sn} - CMD_DOORBELL_NOTIFY_PAYLOAD - Error:`,
              error
            );
          }
          break;
        case CommandType.CMD_NAS_SWITCH:
          try {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - CMD_NAS_SWITCH`,
              { payload: message.data.toString() }
            );
            this.emit(
              "rtsp url",
              ...
              message.data.toString(
                "utf8",
                0,
                message.data.indexOf("\0", 0, "utf8")
              )
            );
          } catch (error) {
            this.log.error(
              `Station ${this.rawStation.station_sn} - CMD_NAS_SWITCH - Error:`,
              error
            );
          }
          break;
        case CommandType.SUB1G_REP_UNPLUG_POWER_LINE:
          try {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - SUB1G_REP_UNPLUG_POWER_LINE`,
              { payload: message.data.toString() }
            );
            const chargeType = message.data
              .slice(0, 4)
              .readUInt32LE() as ChargingType;
            ...
            this.log.error(
              `Station ${this.rawStation.station_sn} - SUB1G_REP_UNPLUG_POWER_LINE - Error:`,
              error
            );
          }
          break;
        case CommandType.SUB1G_REP_RUNTIME_STATE:
          try {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - SUB1G_REP_RUNTIME_STATE`,
              { payload: message.data.toString() }
            );
            ...
            this.log.error(
              `Station ${this.rawStation.station_sn} - SUB1G_REP_RUNTIME_STATE - Error:`,
              error
            );
          }
          break;
        case CommandType.CMD_SET_FLOODLIGHT_MANUAL_SWITCH:
          try {
            ...
            this.log.debug(
              `Station ${this.rawStation.station_sn} - CMD_SET_FLOODLIGHT_MANUAL_SWITCH`,
              { enabled: enabled, payload: message.data.toString() }
            );
            this.emit("floodlight manual switch", message.channel, enabled);
          } catch (error) {
            this.log.error(
              `Station ${this.rawStation.station_sn} - CMD_SET_FLOODLIGHT_MANUAL_SWITCH - Error:`,
              error
            );
          }
          break;
        case CommandType.CMD_GET_DEVICE_PING:
          try {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - CMD_GET_DEVICE_PING`,
              { payload: message.data.toString() }
            );
            this.sendCommandDevicePing(message.channel);
          } catch (error) {
            this.log.error(
              `Station ${this.rawStation.station_sn} - CMD_GET_DEVICE_PING - Error:`,
              error
            );
          }
          break;
        case CommandType.CMD_NOTIFY_PAYLOAD:
          try {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD`,
              { payload: message.data.toString() }
            );
              ...
              if (this.rawStation.station_sn.startsWith("T8520")) {
                ...
                if (
                  ...
                  if (aesKey !== undefined) {
                    ...
                    this.log.debug(
                      `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD Lock - Received`,
                      {
                  ...
                  this.log.debug(
                    `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD - Not implemented`,
                    {
                      commandIdName: CommandType[json.cmd],
                      commandId: json.cmd,
                ...
                switch (payload.lock_cmd) {
                  case 0:
                    if (payload.seq_num !== undefined) {
                      ...
                      this.log.debug(
                        `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD - Lock sequence number`,
                ...
                if (this.deviceSNs[message.channel] !== undefined) {
                  if (payload.lock_payload !== undefined) {
                    ...
                    this.log.debug(
                      `Station ${this.rawStation.station_sn} - CMD_DOORLOCK_DATA_PASS_THROUGH`,
                      {
                        commandIdName: CommandType[json.cmd],
                        commandId: json.cmd,
                        key: key,
                    ...
                    switch (payload.lock_cmd) {
                      case ESLBleCommand.NOTIFY:
                        ...
                        this.emit(
                          ...
                          notifyBuffer.slice(3, 4).readInt8().toString()
                        );
                        this.emit(
                          "parameter",
                          message.channel,
                          CommandType.CMD_DOORLOCK_GET_STATE,
                          notifyBuffer.slice(6, 7).readInt8().toString()
                        );
                        break;
                      default:
                        this.log.debug(
                          `Station ${this.rawStation.station_sn} - CMD_DOORLOCK_DATA_PASS_THROUGH - Not implemented`,
                          { message: message.data.toString() }
                        );
                        break;
                    }
                ...
                if (payload.lock_payload !== undefined) {
                  ...
                  if (fac.getCommandCode() !== ESLBleCommand.NOTIFY) {
                    ...
                    this.log.debug(
                      `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD Lock V12 - Received`,
                      { fac: fac.toString(), aesKey: aesKey }
                    );
                    ...
                    this.log.debug(
                      `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD Lock V12 return code: ${returnCode}`,
                      {
                        commandIdName: CommandType[json.cmd],
                        commandId: json.cmd,
                        decoded: data,
                    ...
                    this.log.debug(
                      `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD Lock V12 - Received notify`,
                      { fac: fac.toString() }
                    );
                  }
                } else {
                  this.log.debug(
                    `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD Lock V12 - Unexpected response`,
                    {
                      commandIdName: CommandType[json.cmd],
                      commandId: json.cmd,
                      message: message.data.toString(),
                ...
                this.log.debug(
                  `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD SmartSafe`,
                  { commandIdName: CommandType[json.cmd], commandId: json.cmd }
                );
                switch (json.cmd) {
                  case CommandType.CMD_SMARTSAFE_SETTINGS: {
                    ...
                    try {
                      ...
                      this.log.debug(
                        `Station ${
                          this.rawStation.station_sn
                        } - CMD_NOTIFY_PAYLOAD SmartSafe return code: ${data.data.readInt8(
                          0
                        )}`,
                        {
                          commandIdName: CommandType[json.cmd],
                      ...
                      this.log.error(
                        `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD SmartSafe Error:`,
                        {
                          commandIdName: CommandType[json.cmd],
                          commandId: json.cmd,
                          channel: this.lastChannel,
                  ...
                  case CommandType.CMD_SMARTSAFE_STATUS_UPDATE: {
                    ...
                    switch (payload.event_type) {
                      case SmartSafeEvent.LOCK_STATUS: {
                        ...
                        this.log.debug(
                          `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD SmartSafe Status update - LOCK_STATUS`,
                      ...
                      default:
                        this.log.debug(
                          `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD SmartSafe Status update - Not implemented`,
                          { message: message.data.toString() }
                        );
                        break;
                    }
                  ...
                  default:
                    this.log.debug(
                      `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD SmartSafe - Not implemented`,
                      { message: message.data.toString() }
                    );
                    break;
                }
            ...
            this.log.error(
              `Station ${this.rawStation.station_sn} - CMD_NOTIFY_PAYLOAD Error:`,
              { error: error, payload: message.data.toString() }
            );
          }
          break;
        case CommandType.CMD_GET_DELAY_ALARM:
          try {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - CMD_GET_DELAY_ALARM :`,
              { payload: message.data.toString("hex") }
            );
            ...
            this.log.error(
              `Station ${this.rawStation.station_sn} - CMD_GET_DELAY_ALARM - Error:`,
              { error: error, payload: message.data.toString("hex") }
            );
          }
          break;
        case CommandType.CMD_SET_TONE_FILE:
          try {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - CMD_SET_TONE_FILE :`,
              { payload: message.data.toString("hex") }
            );
            const alarmEventNumber: AlarmEvent = message.data
              .slice(0, 4)
              .readUInt32LE();
            this.emit("alarm event", alarmEventNumber);
          } catch (error) {
            this.log.error(
              `Station ${this.rawStation.station_sn} - CMD_SET_TONE_FILE - Error:`,
              { error: error, payload: message.data.toString("hex") }
            );
          }
          break;
        case CommandType.CMD_SET_SNOOZE_MODE:
          ...
          try {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - CMD_SET_SNOOZE_MODE`,
              {
                payload: Buffer.from(
                  message.data.toString(),
                  "base64"
            ...
            this.log.error(
              `Station ${this.rawStation.station_sn} - CMD_SET_SNOOZE_MODE - Error:`,
              error
            );
          }
          break;
        ...
        case CommandType.CMD_DATABASE_IMAGE:
          ...
          try {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - CMD_DATABASE_IMAGE`,
            ...
            this.log.error(
              `Station ${this.rawStation.station_sn} - CMD_DATABASE_IMAGE - Error:`,
              error
            );
          }
          break;
        case CommandType.CMD_GET_TFCARD_STATUS:
          try {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - CMD_GET_TFCARD_STATUS :`,
              { payload: message.data.toString("hex") }
            );
            const tfCardStatus: TFCardStatus = message.data
              .slice(0, 4)
              .readUInt32LE();
            this.emit("tfcard status", message.channel, tfCardStatus);
          } catch (error) {
            this.log.error(
              `Station ${this.rawStation.station_sn} - CMD_GET_TFCARD_STATUS - Error:`,
              { error: error, payload: message.data.toString("hex") }
            );
          }
          break;
        case CommandType.CMD_DATABASE:
          try {
            this.log.debug(
              `Station ${this.rawStation.station_sn} - CMD_DATABASE :`,
              { payload: message.data.toString() }
            );
            ...
            switch (databaseResponse.cmd) {
                ...
                for (const record of data) {
                  for (const tableRecord of record.payload) {
                    ...
                    if (record.table_name === "history_record_info") {
                      ...
                      this.log.debug(
                        `Station ${this.rawStation.station_sn} - Not implemented - CMD_DATABASE_QUERY_LOCAL - table_name: ${record.table_name}`
                      );
                    }
              ...
              default:
                this.log.debug(
                  `Station ${this.rawStation.station_sn} - Not implemented - CMD_DATABASE message`,
                  {
                    commandIdName: CommandType[message.commandId],
                    commandId: message.commandId,
                    channel: message.channel,
            ...
            this.log.error(
              `Station ${this.rawStation.station_sn} - CMD_DATABASE - Error:`,
              { error: error, payload: message.data.toString() }
            );
          }
          break;
        default:
          this.log.debug(
            `Station ${this.rawStation.station_sn} - Not implemented - CONTROL message`,
            {
              commandIdName: CommandType[message.commandId],
              commandId: message.commandId,
              channel: message.channel,
  ...
  private async sendAck(
    ...
    await this.sendMessage(
      `Send ack to station ${this.rawStation.station_sn}`,
      address,
  ...
  public async close(): Promise<void> {
    ...
    if (this.socket) {
      if (this.connected) {
        await this.sendMessage(
          `Send end to station ${this.rawStation.station_sn}`,
          this.connectAddress!,
          RequestMessageType.END
        );
        this._disconnected();
  ...
  private scheduleHeartbeat(): void {
    if (this.isConnected()) {
      ...
      this.log.debug(
        `Station ${this.rawStation.station_sn} - Heartbeat disabled!`
      );
    }
  }
  ...
  private scheduleP2PKeepalive(): void {
    if (this.isConnected()) {
      ...
      this.log.debug(
        `Station ${this.rawStation.station_sn} - p2p keepalive disabled!`
      );
    }
  }
  ...
  private async getDSKKeys(): Promise<void> {
    if (this.api.isConnected()) {
      try {
        ...
        this.log.debug(
          `Station ${this.rawStation.station_sn} - Response:`,
          response.data
        );
          ...
          if (result.code == 0) {
            ...
            dataresult.dsk_keys.forEach((key) => {
              if (key.station_sn == this.rawStation.station_sn) {
                ...
                this.log.debug(
                  `${this.constructor.name}.getDSKKeys(): dskKey: ${this.dskKey} dskExpiration: ${this.dskExpiration}`
                );
              }
            });
          } else {
            this.log.error(
              `Station ${this.rawStation.station_sn} - Response code not ok`,
              { code: result.code, msg: result.msg }
            );
          }
        } else {
          this.log.error(
            `Station ${this.rawStation.station_sn} - Status return code not 200`,
            { status: response.status, statusText: response.statusText }
          );
        }
      } catch (error) {
        this.log.error(
          `Station ${this.rawStation.station_sn} - Generic Error:`,
          error
        );
      }
    }
  ...
  private onTalkbackStreamError(error: any): void {
    this.log.debug(
      `Station ${this.rawStation.station_sn} - Talkback Error:`,
      error
    );
  }
  ...
  private async _sendVideoData(message: P2PVideoMessageState): Promise<void> {
    if (message.retries < this.MAX_RETRIES) {
      ...
      this.log.error(
        `Station ${this.rawStation.station_sn} - Max send video data retries ${
          this.messageVideoStates.get(message.sequence)?.retries
        } reached. Discard data.`,
        {
          sequence: message.sequence,
          channel: message.channel,
          retries: message.retries,
    ...
    this.log.debug("Sending p2p video data...", {
      station: this.rawStation.station_sn,
      sequence: message.sequence,
      channel: message.channel,
      retries: message.retries,
    ...
    await this.sendMessage(
      `Send p2p video data to station ${this.rawStation.station_sn}`,
      this.connectAddress!,
      RequestMessageType.DATA,
      message.data
    );
